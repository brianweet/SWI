...
% cmaes.m, Version 3.41.beta.mirrored_serial, last change: March, 2010 
...
defopts.Serial = 'off  % finalize iteration after first better solution is found, mu must be one';
defopts.Mirrored = 'off  % mirror every second offspring, mu should be one';
...
% defopts.CMA.damps = '1 + cs + 2*max(0,sqrt((mueff-1)/(N+1))-1) % damping for step-size';
defopts.CMA.ccum = '4/(N+4)  % cumulation constant for covariance matrix'; 
defopts.CMA.ccov1 = '2 * min(1,lambda/6) / ((N+1.3)^2+mueff)  % learning rate for rank-one update'; 
defopts.CMA.ccovmu = '2 * (mueff-2+1/mueff) / ((N+2)^2+mueff) % learning rate for rank-mu update'; 
% defopts.CMA.ccovmu = '2 * (mueff-1+1/mueff-min(1,20/lambda)) / ((N+2)^2+mueff); 
% possible correction for lambda/mueff large, but not much to gain. Problem also: upper bound needed
% defopts.CMA.ccovmu = '2 * min(N^2/mueff/2, max(1,log(lambda/mueff)-2)) * (mueff-1+1/mueff-1/max(1,lambda/6)) / ((N+2)^2+mueff)'; 
% defopts.CMA.ccovmu = '2 * (mueff-2+1/mueff + min(inf*N^2/mueff/2, max(0,log(lambda/N)))) / ((N+2)^2+mueff)'; 
defopts.CMA.on     = 'yes'; 
...
opts.SaveFilename = strtrim(opts.SaveFilename); % remove trailing white spaces
...
flgSerial = myevalbool(opts.Serial);
flgMirrored = myevalbool(opts.Mirrored);
...
if flgresume % resume is on
  ...
  % More Checking
  if flgMirrored
    if myeval(opts.ParentNumber) > 1
      warning('option Mirrored should not be applied with (default) ParentNumber > 1');
    end
  end
  if flgSerial
    if ~myevalbool(opts.EvalInitialX)
      warning('option Serial should be applied with option EvalInitialX on');
    end
    if flgEvalParallel
      error('with option Serial: set option EvalParallel off, if evaluations count');
    end
    if myeval(opts.ParentNumber) > 1
      error('with option Serial: set option ParentNumber = 1');
    end
  end
...
while isempty(stopflag)
  ...
  arz = randn(N,lambda);
  if flgMirrored  % mirrored vectors must be interlaced, because of the Serial option
    arz(:,2:2:lambda) = -arz(:,1:2:lambda-1);  % should the length be inverted? 
  end
  ...
  if flgEvalParallel
      ...
      if flgSerial && isfinite(fitness.hist(1))
        idx = fitness.raw < fitness.hist(1);
        if any(idx)
          idx = find(idx);
          lambda_act = idx(1);
          if idx(1) < lambda
            fitness.raw(idx(1)+1:end) = inf;
          end
        end
      end
      counteval = counteval + sum(~isnan(fitness.raw(1:lambda_act))); 
      tries = 1;  
  end
  ...
  for k=find(isnan(fitness.raw)), 
    ...
    if flgSerial && fitness.raw(k) < fitness.hist(1) 
      fitness.raw(k+1:end) = inf;
      lambda_act = k;
      break;
    end
  end
...
%             * (repmat(diagC,1,mu) .* arz(:,fitness.idxsel(1:mu)).^2 * weights);
  ...
  % Adapt sigma
  % don't allow too fast increase which can happen with large mueff and damps=1 and small sigma 
  % therefore increase sigma at most with exp(1)
  ...
  out.solutions.recentworst.x = arxvalid(:, fitness.idx(lambda_act));
  out.solutions.recentworst.f = fitness.raw(lambda_act);
  out.solutions.recentworst.evals = counteval + fitness.idx(lambda_act) - lambda;
  if fitness.hist(1) < out.solutions.bestever.f
  ...
  if verbosemodulo > 0 && isfinite(verbosemodulo)
    ...
    if mod(countiter, verbosemodulo) < 1 ...
      ...
      medmax = [median(fitness.raw)-fitness.hist(1) max(fitness.raw)-fitness.hist(1)];
      s1 = num2str(medmax(1), '%.0e '); if ~isfinite(medmax(1)), s1 = [s1 '  ']; end
      s2 = num2str(medmax(2), '%.0e '); if ~isfinite(medmax(2)), s2 = [s2 '  ']; end
...
function f=felli(x, lgscal, expon, expon2)
  persistent countsigns countsigns2
